...
class FlightBotGUI(tk.Tk):
    def __init__(self):
        ...
        start_btn = tk.Button(self, text="Start Monitoring", command=self.start_monitor)
    ...
    def start_monitor(self):
        ...
        try:
            ...
            for name, entry in self.entries.items():
                val = entry.get().strip()
                if name in ("price_limit", "checking_interval", "checking_duration"):
                    params[name] = int(val)
                else:
                    params[name] = val
        except ValueError:
            messagebox.showerror("Invalid input", "Please enter numeric values for price limit, interval, and duration.")
            return
        ...
        thread.start()
        messagebox.showinfo("FlightBot", "Monitoring started in the background.")
        self.quit()
